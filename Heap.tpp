...
private:
    ...
    // Function to maintain the min-heap property by swapping elements up the tree
    void heapifyUp(heapIndex index)
    {
        while (index > 1)
        {
            ...
            {
                std::cout << "Swap positions " << index << "(" << this->tree.at(index) << ") and" << parentIndex << "(" << this->tree.at(parentIndex) << ")." << std::endl;
...
public:
    ...
    void remove(T value)
    {
        ...
        // Replace with last element and remove last node
        this->tree.at(indexToRemove) = this->tree.back();
        this->tree.pop_back();

        // Check how node needs to move
        if (indexToRemove > 1 && this->tree.at(indexToRemove) < this->tree.at(this->getParentPosition(indexToRemove)))
        {
            // Node has to move up
        ...
        {
            // Node has to move down (or stay the same)
